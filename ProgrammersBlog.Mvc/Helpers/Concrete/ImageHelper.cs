...
namespace ProgrammersBlog.Mvc.Helpers.Concrete
{
    public class ImageHelper : IImageHelper
    {
        ...
        public IDataResult<ImageDeletedDto> Delete(string pictureName)
        {
            ...
            var fileToDelete = Path.Combine($"{_wwwroot}/img/", pictureName);
        ...
        {
            if (!Directory.Exists($"{_wwwroot}/img/{folderName}"))
            {
                Directory.CreateDirectory($"{_wwwroot}/img/{folderName}");
            }
            string oldFileName = Path.GetFileNameWithoutExtension(pictureFile.FileName);
            string fileExtension = Path.GetExtension(pictureFile.FileName);
            DateTime dateTime = DateTime.Now;
            string newFileName = $"{userName}_{dateTime.FullDateAndTimeStringWithUnderscore()}{fileExtension}";
            var path = Path.Combine($"{_wwwroot}/img/{folderName}", newFileName);
